...
## 2024-06-05 - Insecure Cookie Deletion and CSRF Token Fixation
**Vulnerability:** The `logout()` function failed to use array syntax for `setcookie()`, preventing the matching `SameSite` attribute from being sent, which can prevent the browser from deleting the session cookie. Additionally, `attemptLogin()` failed to regenerate the CSRF token upon successful authentication, opening the door to Login CSRF (CSRF Token Fixation). Furthermore, `session.use_strict_mode` was missing, allowing session fixation.
**Learning:** Failing to replicate all original cookie parameters (including SameSite) during deletion means the browser may ignore the deletion instruction. Reusing the pre-login CSRF token post-login violates token lifecycle principles. Uninitialized session IDs must be explicitly rejected.
**Prevention:** Always use PHP 7.3+ array syntax in `setcookie()` when deleting cookies to ensure full parameter matching. Explicitly regenerate CSRF tokens alongside session IDs upon authentication state changes. Enforce `ini_set('session.use_strict_mode', 1)`.
