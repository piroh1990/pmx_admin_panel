...
## 2026-04-26 - Insecure Session Fingerprint Validation & Anomalous Session Handling
**Vulnerability:** Session fingerprint verification in `auth.php` used standard comparison operators (`!==`) which are vulnerable to timing attacks. Furthermore, when a session anomaly was detected (e.g., mismatched fingerprint), the application only rejected the validation but did not proactively destroy the session (`session_unset()` and `session_destroy()`), leaving potentially compromised session IDs active on the server.
**Learning:** Cryptographic comparisons for security tokens or session fingerprints must use timing-safe functions to prevent attackers from guessing the expected value byte-by-byte based on response times. Additionally, detecting a session anomaly is a strong indicator of compromise; merely rejecting the validation is insufficient as the session ID remains valid for further abuse.
**Prevention:** Always use `hash_equals()` for comparing cryptographic hashes or tokens. When a session security check fails (like a fingerprint mismatch), proactively destroy the session using `session_unset()` and `session_destroy()` to terminate the potential threat immediately.
