...
## 2026-05-13 - Security Misconfiguration Bypass in `vm_access`
**Vulnerability:** The role-based access control (RBAC) mechanism in `getUserVMs()` used an insecure fallback `isset($USERS[$username]['vm_access']) && is_array(...)` to grant access. If `vm_access` was explicitly configured but intentionally or accidentally malformed (e.g., using a string instead of an array), it silently bypassed the restriction and granted the user access to all VMs.
**Learning:** Security checks that rely on optional configuration must fail securely. Permissive fallbacks meant for backward compatibility can be abused if explicit configuration states are evaluated improperly.
**Prevention:** Always verify explicitly if a security configuration key exists (e.g., using `array_key_exists()`) before evaluating its type. If a security restriction is explicitly declared but invalidly configured, the system must strictly deny access (fail securely) instead of falling back to permissive defaults.
