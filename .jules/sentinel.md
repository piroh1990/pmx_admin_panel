...
## 2026-04-27 - Secure Session Anomaly Handling and Fixation Mitigation
**Vulnerability:** The application was vulnerable to session fixation attacks because `session.use_strict_mode` was not enforced. Additionally, while it detected session anomalies (like user-agent mismatches), it merely returned false without actively destroying the session, leaving compromised session IDs alive. Finally, it did not regenerate the CSRF token upon login, making it vulnerable to Login CSRF.
**Learning:** Relying on boolean returns for session validation checks leaves the session active on the server. If a session is deemed compromised or invalid (e.g., deleted user or fingerprint mismatch), it must be proactively wiped to prevent further abuse. Furthermore, CSRF tokens must be rotated alongside session IDs during privilege escalation (login).
**Prevention:** Always enforce `ini_set('session.use_strict_mode', 1)`. When a session anomaly is detected, proactively call `session_unset()` and `session_destroy()`. Ensure CSRF tokens are actively regenerated (`bin2hex(random_bytes(32))`) upon successful authentication.
