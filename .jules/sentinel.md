...
## 2024-06-05 - Session Token and State Management Vulnerabilities
**Vulnerability:** Several issues were identified: `file_put_contents` in rate limiting lacked `LOCK_EX` causing race conditions; mismatched session fingerprints returned `false` without destroying the session; fingerprint comparison used `!==` allowing timing attacks; and CSRF tokens weren't regenerated after login allowing token fixation.
**Learning:** Security state (like rate limits, session validation, and tokens) requires strict enforcement. Concurrency can bypass rate limits. Failing to destroy compromised sessions leaves them open to further exploitation. Non-constant time string comparisons allow timing attacks. Static CSRF tokens across authentication boundaries lead to fixation attacks.
**Prevention:** Always use `LOCK_EX` when writing state files. Actively destroy (`session_unset(); session_destroy();`) anomalous sessions. Always use `hash_equals()` for comparing security strings. Always regenerate security tokens (like CSRF) alongside the session ID upon authentication.
