...
## 2026-04-25 - Insecure Cookie Invalidation Format
**Vulnerability:** The session invalidation in `logout()` used the legacy signature of `setcookie()` without the array syntax. Because of this, it was impossible to explicitly pass the `samesite` parameter. If `session.cookie_samesite` was strict, the invalidation cookie could be rejected by the browser or processed incorrectly, leaving the old session cookie active in the browser.
**Learning:** `setcookie()` requires PHP 7.3+ array syntax to reliably set attributes like `SameSite`. When invalidating cookies, the path, domain, secure, httponly, and samesite flags must exactly match those of the cookie you intend to delete, otherwise the browser considers it a different cookie.
**Prevention:** Always use `session_get_cookie_params()` alongside PHP array syntax (`setcookie('name', '', ['expires' => ..., 'samesite' => ...])`) to ensure cookie invalidation parameters perfectly match the original cookie's parameters.
