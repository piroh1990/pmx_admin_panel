...
## 2024-04-12 - Session Management Vulnerabilities
**Vulnerability:** Several session management vulnerabilities were identified: session fixation due to missing `session.use_strict_mode`, timing attacks possible during session fingerprint validation, anomalies not destroying sessions immediately, CSRF token fixation, and a race condition during authentication rate-limiting.
**Learning:** Security must be applied in depth across all points of session management. Strict mode, constant-time comparisons, token regeneration on privilege escalation, and proper file locking are non-negotiable for custom PHP applications.
**Prevention:** Enforce `session.use_strict_mode = 1`, always use `hash_equals` for cryptographic tokens, explicitly call `session_destroy()` on anomalies, regenerate both session ID and CSRF tokens upon login, and use `LOCK_EX` when writing to rate-limit files.
