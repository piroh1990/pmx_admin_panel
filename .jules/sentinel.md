...
## 2024-04-07 - Insecure Session Fingerprint Validation
**Vulnerability:** Session fingerprint validation was using the `!==` operator, making it susceptible to timing attacks. Additionally, upon detecting a mismatched fingerprint, the function only returned `false` instead of destroying the session.
**Learning:** This approach leaves the compromised session ID active and vulnerable to continued exploitation, as the attacker could repeatedly attempt to bypass the check. Also, using standard comparison for cryptographic values allows timing attacks.
**Prevention:** Always use `hash_equals()` for comparing cryptographic strings like session fingerprints. Additionally, when a security anomaly like a fingerprint mismatch is detected, proactively destroy the session (`session_unset()` and `session_destroy()`) to neutralize the compromised session ID.
