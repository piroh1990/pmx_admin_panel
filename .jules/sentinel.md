...
## 2026-04-03 - Incomplete Session Cookie Deletion
**Vulnerability:** The session cookie deletion in `auth.php` used the 7-argument `setcookie()` function, which does not support setting the `SameSite` attribute. If the session cookie was created with `SameSite=Strict`, some modern browsers might refuse to delete it unless the deletion cookie also specifies `SameSite=Strict`. This could leave the session cookie in the browser after logout, potentially allowing local session resumption.
**Learning:** In PHP 7.3+, the `SameSite` attribute for cookies can only be set using the array signature of `setcookie()`. When destroying a session cookie, all parameters (including `SameSite`) must exactly match the original cookie to ensure reliable deletion across all browsers.
**Prevention:** Always use `session_get_cookie_params()` and the PHP 7.3+ array signature for `setcookie()` when destroying session cookies, ensuring the `samesite` key is explicitly included.
