...
## 2024-05-31 - IDOR bypass via Fail Open Configuration
**Vulnerability:** The `getUserVMs()` function in `auth.php` had a vulnerability where if `vm_access` was explicitly defined for a user but was invalidly configured (e.g., set to a string instead of an array), the function would fail to process it and silently fall back to granting the user access to all VMs (`$VMS`). This fail-open behavior led to an Insecure Direct Object Reference (IDOR) bypass.
**Learning:** Security configurations must never fail open. If a specific restriction is attempted but is malformed or invalid, falling back to a permissive default inadvertently elevates privileges.
**Prevention:** Always implement fail-secure logic. If an optional security configuration is present but invalid, strictly deny access instead of defaulting to a permissive fallback.
