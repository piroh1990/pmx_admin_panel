...
## 2024-06-01 - Insecure Session Management & Login CSRF
**Vulnerability:** Several issues were identified with session management in `auth.php`: `session.use_strict_mode` was not enabled (allowing Session Fixation), the CSRF token was not regenerated on successful login (allowing Login CSRF), and the `logout` function used the legacy `setcookie()` syntax which failed to set the `SameSite` attribute when invalidating the cookie.
**Learning:** Hardened session settings must be enforced consistently. If `strict_mode` is off, an attacker can force a known session ID. If a CSRF token isn't rotated on login, an attacker can perform a Login CSRF attack. If a session cookie is deleted without the `SameSite` attribute, the deletion may not be processed correctly by strict browsers, leaving the session active.
**Prevention:** Enable `session.use_strict_mode = 1` in configuration. Actively regenerate CSRF tokens alongside session IDs during authentication state changes. Always use the array syntax for `setcookie()` (introduced in PHP 7.3) to ensure all cookie attributes, including `SameSite`, are correctly managed.
