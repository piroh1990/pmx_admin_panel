...
## 2024-05-31 - Active Session Destruction on Anomaly
...
## 2026-05-12 - Authorization Bypass via Fail-Open Optional Configuration
**Vulnerability:** The `vm_access` array in the user configuration is optional. In `getUserVMs()`, the presence check `isset($USERS[$username]['vm_access']) && is_array($USERS[$username]['vm_access'])` evaluates to `false` if the `vm_access` configuration exists but is incorrectly formatted as a string/integer instead of an array. Because it evaluates to false, it skips the restriction logic and falls through to a permissive default (`return $VMS;`), granting the user access to all VMs.
**Learning:** Checking for configuration validity and presence in a single conditional (`isset && is_array`) can lead to fail-open vulnerabilities. If a security restriction is explicitly declared (even if misconfigured), failing to recognize it and falling back to permissive defaults allows authorization bypass.
**Prevention:** Security restrictions must fail securely. When evaluating optional configurations like access arrays, always explicitly verify if the configuration key exists using `array_key_exists()`. If the key exists but the data structure is invalid, explicitly deny access (fail-closed) rather than ignoring the restriction.
