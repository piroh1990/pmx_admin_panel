...
## 2024-05-24 - Fix missing explicit SSL host verification in cURL
...
## 2026-04-20 - Defense-in-Depth Authentication Enhancements
**Vulnerability:** Several subtle vulnerabilities existed in the authentication flow: uninitialized sessions could be fixated ( missing), timing attacks were possible against the session fingerprint ( instead of ), compromised sessions weren't proactively destroyed, CSRF tokens could be fixated on login, and rate-limiting file writes were subject to race conditions.
**Learning:** Security mechanisms often have subtle configuration gaps. PHP's default session behavior allows fixation if strict mode isn't enabled. Standard comparison operators leak timing information. Session state needs active invalidation, not just passive rejection. Tokens must cycle with trust boundaries (like login). File-based state needs explicit locking ().
**Prevention:** Always pair  with CSRF token regeneration upon login. Use  for all security-sensitive string comparisons. Proactively call  and  on security anomalies. Enable . Use  when writing to shared state files.

## 2026-04-20 - Defense-in-Depth Authentication Enhancements
**Vulnerability:** Several subtle vulnerabilities existed in the authentication flow: uninitialized sessions could be fixated (`session.use_strict_mode` missing), timing attacks were possible against the session fingerprint (`!==` instead of `hash_equals()`), compromised sessions weren't proactively destroyed, CSRF tokens could be fixated on login, and rate-limiting file writes were subject to race conditions.
**Learning:** Security mechanisms often have subtle configuration gaps. PHP's default session behavior allows fixation if strict mode isn't enabled. Standard comparison operators leak timing information. Session state needs active invalidation, not just passive rejection. Tokens must cycle with trust boundaries (like login). File-based state needs explicit locking (`LOCK_EX`).
**Prevention:** Always pair `session_regenerate_id(true)` with CSRF token regeneration upon login. Use `hash_equals()` for all security-sensitive string comparisons. Proactively call `session_unset()` and `session_destroy()` on security anomalies. Enable `session.use_strict_mode = 1`. Use `LOCK_EX` when writing to shared state files.
