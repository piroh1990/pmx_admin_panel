...
## 2024-05-31 - Session and Authentication Hardening
**Vulnerability:** Several minor authentication gaps existed: missing strict mode enabled session fixation, lack of HSTS preload hindered browser security guarantees, CSRF tokens weren't regenerating on login (Token Fixation/Login CSRF risk), rate-limit updates lacked file locks (`LOCK_EX`) causing race conditions, session fingerprints used standard comparison (`!==`) instead of `hash_equals()` creating a timing leak, and `logout()` used legacy `setcookie` syntax which drops `SameSite` flags.
**Learning:** Defense in depth requires continuous refinement of standard mechanisms. Even with session management implemented, missing flags (strict mode, preload, SameSite on delete), non-constant-time comparisons, and race conditions in temporary files can introduce subtle security flaws under specific attack vectors.
**Prevention:** Always enable `session.use_strict_mode`, use `hash_equals()` for all security comparisons, proactively destroy sessions upon anomaly detection, enforce `LOCK_EX` on shared file state, regenerate both Session IDs and CSRF tokens on privilege boundaries, and use PHP 7.3+ array syntax for all `setcookie` calls to guarantee attribute preservation.
