...
## 2026-05-15 - Fail Securely on Invalid Authorization Config
**Vulnerability:** `getUserVMs()` in `auth.php` used `isset($array['vm_access']) && is_array($array['vm_access'])`. If `vm_access` was defined as a string due to misconfiguration, it bypassed the check and granted full access to all VMs.
**Learning:** Security checks that rely on optional configuration must fail securely. If a security restriction (like a vm_access array) is explicitly declared but invalidly configured, the system must strictly deny access instead of falling back to permissive defaults.
**Prevention:** When evaluating optional array-based access restrictions, explicitly check `array_key_exists()` and halt execution or return no access if the data type is incorrectly configured.
