...
## 2024-05-31 - Session Fingerprint Timing Attack and Insecure Anomaly Handling
**Vulnerability:** Session fingerprint verification in `auth.php` used standard comparison (`!==`) instead of `hash_equals()`, exposing it to timing attacks. Furthermore, on a fingerprint mismatch, it only returned `false` without destroying the session. An attacker with a compromised session ID could retain the session on the server while potentially attempting to spoof the user agent or exploit the timing side-channel.
**Learning:** Security tokens, hashes, and fingerprints must be compared using constant-time algorithms (`hash_equals()`) to prevent attackers from inferring the expected value. Additionally, when a strong session anomaly is detected (like a changed user agent), merely rejecting the request is insufficient; the session must be proactively invalidated to stop ongoing attacks.
**Prevention:** Always use `hash_equals()` for comparing sensitive tokens/hashes. Implement proactive session destruction (`session_unset()` and `session_destroy()`) immediately upon detecting session anomalies or suspected hijacking attempts.
