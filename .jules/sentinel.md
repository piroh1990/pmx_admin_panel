...
## 2024-05-32 - Fail-Open Authorization Bypass via Invalid Configuration
**Vulnerability:** The `getUserVMs()` function fell back to permissive defaults (granting access to all VMs) when the `vm_access` restriction was explicitly declared but invalidly configured (e.g., not an array).
**Learning:** Security checks that rely on optional configurations must fail securely. If a configuration meant to restrict access is present but malformed, the system should interpret it as a secure restriction (deny all) rather than ignoring it and applying permissive defaults.
**Prevention:** Explicitly verify if a restriction key exists (`array_key_exists`) and if it is malformed, explicitly return an empty/denied state rather than continuing to the fallback logic.
