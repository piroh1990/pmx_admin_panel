...
## 2026-04-08 - Session Fingerprint Bypass via Timing and Replay
**Vulnerability:** The session fingerprint validation in `isLoggedIn()` used standard comparison (`!==`) instead of `hash_equals()`, making it vulnerable to timing attacks. Furthermore, on failure, it only returned `false` instead of actively destroying the compromised session, allowing an attacker to continue attempting to use the session ID.
**Learning:** Returning `false` without destroying the session leaves the session active server-side. An attacker with a hijacked session cookie can repeatedly try different User-Agents until they find the matching one, aided by the timing leak from the `!==` operator.
**Prevention:** Always use `hash_equals()` for cryptographic or token comparisons. Proactively destroy sessions (`session_unset()` and `session_destroy()`) when an anomaly like a mismatched fingerprint is detected to kill the compromised session immediately.
