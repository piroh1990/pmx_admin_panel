...
## 2026-04-16 - Timing Leak and Session Continuity on Fingerprint Mismatch
**Vulnerability:** The session fingerprint validation in `auth.php` (`isLoggedIn()`) used the standard `!==` operator to compare the calculated `hash('sha256')` of the User-Agent against the stored `$_SESSION['fingerprint']`. This exposed a timing leak that could theoretically allow attackers to forge session fingerprints. Furthermore, if a mismatch occurred, the function simply returned `false` without proactively destroying the session, leaving the compromised session alive and potentially allowing it to be reused if the attacker guessed or spoofed the fingerprint correctly later.
**Learning:** Security tokens and session identifiers (like fingerprints) must be compared using `hash_equals()` to prevent timing attacks. Additionally, when an anomaly is detected that strongly suggests session hijacking (such as a fingerprint mismatch), the session must be actively destroyed (`session_unset()` and `session_destroy()`), not just rejected for that single request.
**Prevention:** Always use `hash_equals()` for cryptographic comparisons and aggressively invalidate sessions upon detecting tampering or hijacking attempts.
