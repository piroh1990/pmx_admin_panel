...
## 2026-04-11 - Proactive Session Destruction and Timing Attack Mitigation in Fingerprinting
**Vulnerability:** The session fingerprint validation routine in `isLoggedIn()` correctly verified the user agent, but used a non-constant time comparison (`!==`) and merely returned `false` without proactively destroying the session upon detecting a mismatch.
**Learning:** This implementation allowed an attacker who stole a session cookie to repeatedly guess the user agent fingerprint because the server didn't explicitly terminate the compromised session, relying on passive expiration. The use of `!==` also introduced a theoretical timing side-channel.
**Prevention:** Always use `hash_equals()` for cryptographic comparisons like session fingerprints. Furthermore, if an anomaly is detected (like a mismatched fingerprint indicating a potential hijacking attempt), actively destroy the session using `session_unset()` and `session_destroy()` to neutralize the compromised session ID immediately and prevent further abuse.
