...
## 2024-05-31 - Secure Default Fallback in Authorization
**Vulnerability:** `getUserVMs()` in `auth.php` had a misconfiguration vulnerability. If a user's `vm_access` was explicitly defined but not an array (e.g., a single string ID instead of an array of IDs), the array check failed, and the function dangerously fell back to returning all VMs.
**Learning:** When falling back to a default configuration (like "allow all" for backward compatibility), you must distinguish between an "undefined" setting (where the fallback applies) and an "invalid/misconfigured" setting (where it must fail securely). Relying on a combined `isset() && is_array()` check causes misconfigurations to bypass the explicit restriction entirely.
**Prevention:** Always evaluate the presence of a security configuration first (`isset()`). If it is present, treat it as the absolute authority. If it is invalid (e.g., wrong type), fail securely and deny access rather than falling back to permissive defaults.
