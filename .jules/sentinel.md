...
## 2026-05-14 - Array Configuration Bypass & CSRF Fixation
**Vulnerability:** A logic flaw in `isset($arr['key']) && is_array($arr['key'])` allowed users to bypass `vm_access` restrictions by supplying a non-array (e.g. a string). Additionally, the CSRF token was not regenerated on login (Login CSRF).
**Learning:** Optional security configuration must explicitly check `array_key_exists` and securely fail closed (`return []`) if the type is incorrectly configured, instead of falling through to permissive defaults.
**Prevention:** Always use `array_key_exists()` for optional arrays and regenerate CSRF tokens on state-changing authentications.
